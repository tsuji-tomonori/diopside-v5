...
v0.4 正本との差分は `docs/design/dynamodb-schema-audit.md` に整理しています。現 repository は single-table と S3 退避方針は近い一方、key prefix、`schema_version`、`ChannelRef`、`VideoMonthIndex` などに未対応または差分があります。`NotificationPlan`、`TagSummary`、`RandomBucket`、`StaticExport` は v0.4 key shape で部分実装しています。
