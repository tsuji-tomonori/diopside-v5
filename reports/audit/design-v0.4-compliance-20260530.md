...
| P0-08 | DDB schema | v0.4 item type と現 repository contract の差分を `docs/design/dynamodb-schema-audit.md` に整理し、主要 writer の current schema を test 化。`RandomBucket` writer/query path を追加 | 監査済み・差分あり | key prefix / schema_version / 残未対応 item の実装は後続 |
