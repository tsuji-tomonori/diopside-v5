...
API-020 は互換用の call record `items` に加え、daily summary 由来の `daily`、`by_method`、`limit_per_day`、`warning` を返すよう更新した。管理 UI の quota panel は現時点では call record 表示を維持する。
...
| P1 | quota rollup | 対応 | `quota_rollup` が call record から v0.4 key shape の daily method summary `QuotaUsage` item を保存し、閾値超過時に `quota_threshold_warning` JobEvent を記録。API-020 は daily/by_method/warning を返す。外部通知 delivery、管理 UI daily summary 表示、CloudWatch Alarm は後続 |
