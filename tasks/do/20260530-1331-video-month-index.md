...
- [ ] `VideoMonthIndex` item が `pk=VID#{video_id}`, `sk=INDEX#MONTH#{yyyyMM}` で保存される。
- [ ] item に `video_id`、`yyyy_mm`、`published_at`、`title`、`thumbnail_url`、`duration_sec`、`archive_state` が含まれる。
- [ ] item は `gsi1pk=VIDEO#MONTH#{yyyyMM}`, `gsi1sk=PUB#{published_at}#{video_id}` を持つ。
- [ ] `put_video` により公開動画の index が更新され、非公開化または公開月変更時に stale index が削除される。
- [ ] `GET /api/archive-calendar` が repository 利用時に `VideoMonthIndex` read model を利用する。
- [ ] static export の `/data/calendar/{year}.json` が repository の calendar read model を利用できる。
- [ ] README、traceability、DDB schema audit、compliance audit が更新される。
