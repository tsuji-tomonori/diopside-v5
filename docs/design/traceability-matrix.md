...
| design_id | category | requirement | implementation_files | tests | status |
|---|---|---|---|---|---|
| DDB-SCHEMA | DynamoDB | v0.4 item schema と key/GSI 設計。`VideoMonthIndex` と `TagSummary` は read model として部分実装済み。 | `apps/shared/src/diopside_core/repository.py`, `README.md`, `docs/design/dynamodb-schema-audit.md` | `tests/test_repository_schema_contract.py`, `tests/test_core_pipeline.py`, `tests/test_static_exporter.py`, `tests/test_cloudformation_contract.py` | 監査済み・差分あり |
