...
| BATCH-020 | 管理手動ジョブディスパッチ | admin job API / EventBridge Scheduler | 各 queue | `tests/test_api_handler.py`, `tests/test_core_pipeline.py`, `tests/test_cloudformation_contract.py`, `tools/run-local-e2e.mjs` | 部分実装 | 管理 API、Scheduler template、worker 後続投入は v0.4 `JobMessage` field を送信。GitHub Actions dispatch は未整合 |
...
- 管理 API、EventBridge Scheduler template、worker の後続投入は v0.4 `JobMessage` field として `job_id`、`job_type`、`idempotency_key`、`requested_by`、`attempt`、`trace_id`、`payload` を送る。`dispatch_job` は既存外部 producer 互換のため旧 `input` field も受け付ける。
...
6. GitHub Actions workflow_dispatch 由来の message fields を v0.4 `JobMessage` 共通 schema へ統一する。
